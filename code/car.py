...
class CarlaReliableTransferEnv(gym.Env):
    ...
    def step(self, action: np.ndarray):
        ...
        # The replay action is the command selected by the policy.  Deadzones,
        # mutual exclusion, slew limiting, and an optional safety filter are
        # environment dynamics; storing the post-filter action would train Q
        # on a different action distribution than the actor optimizes.
...
class GraphAttention(nn.Module):
    ...
    def forward(self, edges: torch.Tensor, mask: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if self.score_form == "paper_ratio":
...
class ReplayBuffer:
    ...
    def add(
        ...
        # A time-limit/stuck truncation still has a valid continuation value.
        # Only true MDP termination suppresses SAC bootstrapping.
...
class SACAgent:
    ...
    def update(
        ...
        if source_agent is not None and source_batch is not None:
...
def build_reward_from_info(info: Dict[str, object], sigma_bar: float, cfg: Config = CFG) -> float:
    """Paper-faithful dense reward from Eqs. (8)–(9).

    r_t = w_s r_s + w_p r_p + w_c r_c + w_u r_u    (Eq. 8)
    r_u = 1 − σ̄                                     (Eq. 9)

    No terminal bonus/penalty (goal_bonus = collision_penalty = 0):
    the constrained shaping of Eq. (4) is handled through continuous smooth
    ...
    r_dense = float(cfg.w_s * rs + cfg.w_p * rp + cfg.w_c * rc + cfg.w_u * ru)
    ...
    try:
        ...
        for _ in range(max_total_steps):
            ...
            try:
                ...
                obs = None
                continue
            command_action = np.asarray(info.get("command_action", action), dtype=np.float32)
            reward, _ = recompute_agent_reward(agent, obs, command_action, info, cfg=agent.cfg)
            buf.add(obs, command_action, reward, next_obs, bool(terminated))
            episode_steps += 1
            if terminated or truncated:
                episodes_done += 1
                print(
...
def train_loop(
    ...
    try:
        while True:
            ...
            env_steps += 1
            command_action = np.asarray(info.get("command_action", action), dtype=np.float32)
            reward, sigma_bar = recompute_agent_reward(agent, obs, command_action, info, cfg=cfg)
            info["sigma_bar"] = float(sigma_bar)
            done = bool(terminated or truncated)
            replay.add(obs, command_action, reward, next_obs, bool(terminated))
            ...
            if source_agent is not None and source_env is not None and source_replay is not None and source_env_builder is not None:
                ...
                try:
                    ...
                    source_command_action = np.asarray(source_info.get("command_action", source_action), dtype=np.float32)
                    source_reward, source_sigma_bar = recompute_agent_reward(
                        source_agent, source_obs, source_command_action, source_info, cfg=source_agent.cfg
                    )
                    source_info["sigma_bar"] = float(source_sigma_bar)
                    source_done = bool(source_terminated or source_truncated)
                    source_replay.add(source_obs, source_command_action, source_reward, source_next_obs, bool(source_terminated))
...
def run_target_adaptation(args: argparse.Namespace, cfg: Config, device: torch.device) -> None:
    ...
    if int(args.source_port) == int(args.port):
        raise ValueError(
            "adapt mode needs a second CARLA server for source replay; "
            "set --source-port to a port different from --port"
        )
    if int(args.source_tm_port) == int(args.tm_port):
        raise ValueError(
            "adapt mode needs a distinct Traffic Manager for the source server; "
            "set --source-tm-port differently from --tm-port"
        )
    ...
    if int(args.adapt_maml_warmup_batches) > 0:
